...
namespace Aspose.Pdf.Cloud.Live.Demos.UI.Models
{
	public class ViewModel
	{
		...
		public bool SaveAsComponent
		{
			...
			set
			{
				...
				if (_saveAsComponent)
				{
					...
					if ("pdf".Equals(Product.ToLower()) || "page".Equals(Product.ToLower()))
					{
						sokey2 = $"{Product}ConversionSaveAsExtensionsMvc";
					}
					if ("cells".Equals(Product.ToLower()))
						{
						sokey2 = $"{Product}{AppName}SaveAsExtensions";
					}
		...
		public string SaveAsOptionsList { get {
				string list = "";
