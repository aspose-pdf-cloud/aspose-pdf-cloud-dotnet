...
namespace Aspose.Pdf.Cloud.Live.Demos.UI.Config
{
    ...
    public class GlobalAppHelper
    {
		...
		public GlobalAppHelper(HttpContext hc, HttpApplicationState appState, string SessionID, string language)
		{
			...
			AsposePDFCloudContext.atcc = new AsposePDFCloudContext(hc); // sync the context
		...
		private void initResources(string ResourcesFile, string SessionID)
		{
			...
			if (AsposePDFCloudContext.Current.Cache[SessionID] == null)
			{
				...
				AsposePDFCloudContext.Current.Cache.Add(
				   SessionID,
					resources,
